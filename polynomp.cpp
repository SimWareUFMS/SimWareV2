...
double forecast_errorPolynomp(unsigned long i0,unsigned long i1)
{
  unsigned int i;
  unsigned long n;
  double h,error=0.0;
...
void make_castPolynomp(FILE *fcast)
{
  int i,j,hi;
  unsigned int k;
  double casted;
...
int runPolynomp(int argc,char **argv)
{
  int i,j;
  char stdi=0,oose=1;
  ...
  make_fitPolynomp();
  withalli=forecast_errorPolynomp(0LU,insample);
  withallo=0.0;
  if (oose)
    withallo=forecast_errorPolynomp(insample+1,length);
